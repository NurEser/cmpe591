...
def collect(idx, N):
    ...
    init_imgs = torch.zeros(N, 3, 128, 128, dtype=torch.uint8)
    final_imgs = torch.zeros(N, 3, 128, 128, dtype=torch.uint8) 

    # Create a dedicated folder to keep your project root clean
    save_dir = "dataset"
    os.makedirs(save_dir, exist_ok=True)

    for i in range(N):
        # 1. Capture Initial State
        _, pre_action_pixels = env.state()
        ...
        post_action_pos, post_action_pixels = env.state()
        
        # 4. Save to RAM
        init_imgs[i] = pre_action_pixels
        actions[i] = action_id
        positions[i] = torch.tensor(post_action_pos)
        final_imgs[i] = post_action_pixels
        
        env.reset()

        if (i + 1) % 10 == 0:
            print(f"Process {idx} finished {i + 1}/{N} simulations.")

    # 5. Save all files directly into the 'dataset' folder
    print(f"Process {idx} saving files to '{save_dir}/'...")
    
    torch.save(positions, os.path.join(save_dir, f"positions_{idx}.pt"))
    torch.save(actions, os.path.join(save_dir, f"actions_{idx}.pt"))
    torch.save(init_imgs, os.path.join(save_dir, f"init_imgs_{idx}.pt"))
    torch.save(final_imgs, os.path.join(save_dir, f"final_imgs_{idx}.pt"))
    
    print(f"Process {idx} complete!")


if __name__ == "__main__":
    processes = []
    # 4 processes * 250 samples each = 1000 total samples
    for i in range(4):
    ...
    print("All 1000 data points successfully collected and saved in the 'dataset' folder!")